...
load(file = "/path/to/F7-lm-NoOut-heightcut-full450k.RData");
lnames = load(file = "/path/to/cord_heightcut_WGCNA_full450k.RData");
cordModuleData = read.csv(file = "/path/to/birth_GeneInfo.csv") # module assignment info in here
...
# Save the results
save(mp, file = "/path/to/cordModulePreservationInF7.Rdata")
...
# Start the plot
sizeGrWindow(10, 5);
pdf(file="/path/to/cordModulesInF7-modulePreservation-Zsummary-medianRank.pdf", wi=10, h=5)
par(mfrow = c(1,2))
...
sizeGrWindow(12, 9);
pdf(file="/path/to/cordModulesInF7-modulePreservation-moduleQualityAndPreservation.pdf", wi=10, h=5)
par(mfrow = c(3,5))
par(mar = c(3,3,2,1))
par(mgp = c(1.6, 0.4, 0));
